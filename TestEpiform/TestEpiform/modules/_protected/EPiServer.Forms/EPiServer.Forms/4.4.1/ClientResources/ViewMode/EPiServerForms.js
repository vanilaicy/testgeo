    ...
    $.extend(true, epi.EPiServer.Forms, {
        Utils: {
            ...
            showNextStepOnEnterKeyDown: function (e) {
                var $workingForm = $(e.target).parents(".EPiServerForms:first");
                keyCode = e.keyCode,
        ...
        $(document).ready(function () {
            ...
            function _doSubmitForm(/*Event*/e) {
                ...
                $.ajax({
                    ...
                    async: epi.EPiServer.Forms.AsyncSubmit || $workingForm.data('epiforms-async-submit') || false,  // by default, form will submit synchronously, prevent Visitor continuously hit submit button (by accident)
            ...
            function showServerValidationMessage(workingFormInfo, validationInfo) {
                ...
                for (var i = 0; i < validationInfo.length; i++) {
                    if (validationInfo[i] && validationInfo[i].InvalidElement) {
                        ...
                        var $messageContainer = _getValidationMessage($element),
                            validatorsInfo = _getElementValidators(workingFormInfo.ValidationInfo, validationInfo[i].InvalidElement),
                            validatorInfo = _getValidatorByValidatorType(validatorsInfo, validationInfo[i].Validator);
                ...
                if ($element) {
                    _refreshCaptchaElement($element.parents(".FormStep:first"));
                }
            }
                ...
                $.each(formData, function (name, val) {
                    ...
                    if ($item.length > 0) { //found the input control that should hold field value
                        ...
                        if ($item.hasClass("FormFileUpload__Input")) {
                            var postedFileNames = "";
                            var $fileUploadElement = $item.parents(".FormFileUpload:first");
            ...
            function _scrollToTheTopOfForm() {
                var formsTitle = $(".Form__Title");
                if ((formsTitle.css("position").toLowerCase() !== "fixed") && ($(document).scrollTop() > 0)) {
                    window.scrollTo(0, formsTitle.offset().top);
                }
            }
